...
document.addEventListener('DOMContentLoaded', function () {
    ...
    createColorButtons('color1');
    createColorButtons('color2');
    createColorButtons('color3');
});
...
function selectColor(colorPickerId, colorName, button) {
    const colorButtonClass = `f1-${colorName}`;
    const parentElement = button.parentElement;
    parentElement.className = `color-picker ${colorButtonClass}`;

    // update available Colors
    selectedColor1 = availableColors.find(color => color.name === 'blue');
    ...
    if (colorPickerId === 'Farbe1') {
        selectedColorPicker1 = colorName;
        if (colorName === 'rot') {
    ...
    localStorage.setItem(colorPickerId, colorName);
    updateSelectedColor(button);
    updateKonfigColor(colorName);
}
